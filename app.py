"""Buzzwords & Misdemeanors - HF Space entrypoint.

Run locally:  python app.py
The UI launches even without weights; it then tells you which GGUFs to add to models/
and where it expects the llama-server binary (see buzzwords/config.py).
"""

import logging

# Log to stdout so HF Spaces' Container logs capture it. INFO surfaces generation
# progress; full tracebacks are logged on any beat failure.
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s %(levelname)s [%(name)s] %(message)s",
    force=True,   # override any handler a dependency installed at import time
)

from buzzwords import config
from buzzwords.theme import get_css
from buzzwords.ui import build_ui

# On HF Spaces / fresh machines, pull the GGUFs from the Hub at startup
# (base model + director/style LoRAs) instead of committing them to the repo.
if config.FETCH_WEIGHTS:
    from buzzwords.weights import ensure_weights
    ensure_weights()

demo = build_ui()

if __name__ == "__main__":
    demo.queue(default_concurrency_limit=1).launch(   # engine is further serialized internally
        server_name="0.0.0.0",   # Docker Space: bind the public port (HF expects 7860)
        server_port=7860,
        css=get_css(),
        allowed_paths=[str(config.MAPS_DIR)],   # serve the courtroom backdrops
    )
